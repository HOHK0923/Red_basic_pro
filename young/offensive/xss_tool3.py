...
class XSSAttackToolV3:
    def __init__(self, target_url, attacker_server, use_tor=False,stealth_mode=False, proxy_port=None):
    ...
    def setup_tor(self):
        ...
        try:
            ...
            try:
                ...
                print(f"{Fore.GREEN}[+] Connected to Tor. Current IP: {ip_check}{Style.RESET_ALL}")
            except:
                print(f"{Fore.YELLOW}[!] Could not verify Tor IP{Style.RESET_ALL}")
                
        except Exception as e:
            print(f"{Fore.RED}[-] Tor setup failed: {e}{Style.RESET_ALL}")
            print(f"{Fore.YELLOW}[!] Make sure Tor is running: brew services start tor{Style.RESET_ALL}")
            self.use_tor = False
...
{Fore.YELLOW}Tor:{Style.RESET_ALL} {'ON' if self.use_tor else 'OFF'}
...
{Fore.MAGENTA}[T]{Style.RESET_ALL} New Tor Identity          - Get new IP address (Tor only)
